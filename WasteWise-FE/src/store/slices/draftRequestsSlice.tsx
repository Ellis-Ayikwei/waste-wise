...
const draftRequestsSlice = createSlice({
    ...
    reducers: {
        addDraft: (state, action: PayloadAction<{ id: string; data: Partial<ServiceRequest> }>) => {
            ...
            // Enforce up to 3 local drafts: remove the oldest local draft if capacity reached
            const localDrafts = state.drafts.filter((d) => d.source === 'local');
            if (localDrafts.length >= 3) {
                const oldestLocal = [...localDrafts].sort((a, b) => (a.lastModified || a.createdAt).localeCompare(b.lastModified || b.createdAt))[0];
                state.drafts = state.drafts.filter((d) => !(d.source === 'local' && d.id === oldestLocal.id));
            }
