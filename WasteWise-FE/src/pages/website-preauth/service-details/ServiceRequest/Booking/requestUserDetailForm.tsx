...
import { setBookingDetails } from '../../../../../store/slices/createRequestSlice';
...
const BookingDetailsForm: React.FC<BookingDetailsFormProps> = ({ selectedPrice, requestId, onBack, initialPostcode, onComplete }) => {
    ...
    const searchPostcodeSuggestions = async (query: string, stopIndex: number) => {
        ...
        try {
            ...
            if (response.status === 200 && response.data.suggestions && response.data.suggestions.length > 0) {
                setAddressStates((prev) => ({
                    ...prev,
                    [stopIndex]: {
                        ...prev[stopIndex],
                        postcodeOptions: response.data.suggestions,
    ...
    const fetchAddressesForPostcode = async (postcode: string): Promise<AddressOption[]> => {
        try {
            const response = await axiosInstance.get(`/geocoding/postcode-addresses-demo/`, {
                params: { postcode },
            });

            if (response.status === 200 && response.data.addresses && response.data.addresses.length > 0) {
                const addresses: AddressOption[] = response.data.addresses.map((addr: any) => ({
    ...
    const validatePostcode = async (postcode: string): Promise<{ isValid: boolean; message?: string }> => {
        try {
            ...
            // const response = await axiosInstance.get(`/geocoding/validate-postcode/`, { params: { postcode: cleanPostcode } });
            // if (response.status === 200 && response.data.isValid) {
            //     return { isValid: true };
            // } else {
            //     return { isValid: false, message: response.data.message || 'Invalid postcode' };
            // }
            return { isValid: true };
        } catch (error) {
