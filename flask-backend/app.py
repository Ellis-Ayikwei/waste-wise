...
def register_bin_legacy():
    ...
    try:
        ...
        print(f"✅ Bin registered (legacy): {bin_id} at {bin_record['location'].get('address', 'Unknown location')}")
...
def receive_bin_data_legacy(bin_id):
    ...
    try:
        ...
        sensor_reading = {
            ...
            "tilt": data.get('sensor_data', {}).get('tilt', 0) > 10,  # Convert tilt degrees to boolean
...
def register_bin():
    ...
    try:
        ...
        print(f"✅ Bin registered: {bin_id} at {bin_record['location'].get('address', 'Unknown location')}")
...
def cleanup_database():
    ...
    try:
        ...
        print(f"🧹 Cleaned up {deleted_count} old sensor readings (older than {days} days)")
