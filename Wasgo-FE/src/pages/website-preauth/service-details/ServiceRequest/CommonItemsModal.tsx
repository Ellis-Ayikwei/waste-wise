...
import { formatCategoryString, getItemIcon } from '../../../../utilities/getItemIcon';
...
function getAllowedCategoriesByServiceTitle(serviceTitle: string): string[] {
    switch (serviceTitle) {
...
const CommonItemsModal: React.FC<CommonItemsModalProps> = ({ isOpen, onClose, onSelectItem, service }) => {
    if (!isOpen) return null;
    const { data: categories, isLoading } = useSWR(isOpen ? '/item-categories/' : null, fetcher, swrOptions);
    // Replace SWR for commonItems with axiosInstance
    const [commonItems, setCommonItems] = useState<any>(null);
    const [comLoading, setComLoading] = useState(false);
    useEffect(() => {
        if (isOpen) {
            setComLoading(true);
            axiosInstance.get('/common-items/categories_with_items/')
                .then(res => setCommonItems(res.data))
                .catch(() => setCommonItems(null))
    ...
    const allowedCategories = getAllowedCategoriesByServiceTitle(service);
    console.log("the allowed ats", allowedCategories)
    const filteredCategories = useMemo(() => {
        ...
        return commonItems
            .map((category: any) => {
                ...
                if (allowedCategories.length > 0 && !allowedCategories.includes(category.name)) {
                    items = [];
                }
                // Filter by search
                if (search.trim()) {
            ...
            .filter(Boolean);
    }, [commonItems, allowedCategories, search]);
            ...
            <div className="flex items-center justify-center min-h-screen p-4">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full overflow-hidden transform transition-all" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="bg-gray-50 dark:bg-gray-750 px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                        <h3 className="text-lg font-medium text-gray-900 dark:text-white">Quick Add Common Items for {service}</h3>
                    ...
                    <div className="p-6">
                        ...
                        <div className="mb-4">
                            ...
                            />
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:flex lg:flex-wrap lg:justify-center gap-3 p-1 text-wrap">
                                {/* Category buttons */}
                                {categories?.map((category: any, idx: number) => (
                                    <button
                                        ...
                                        className={`${getItemIcon(category.name).tabColor} px-1 lg:px-4 py-2 rounded-md text-xs sm:text-sm text-center min-h-[40px] break-words`}
                                    >
