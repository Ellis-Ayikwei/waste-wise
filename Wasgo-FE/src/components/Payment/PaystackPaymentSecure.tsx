...
const PaystackPaymentSecure: React.FC<PaystackPaymentProps> = ({
  amount,
  email,
  requestId,
  onSuccess,
  ...
  // Initialize payment with backend (backend handles Paystack)
  ...
  // Charge saved card (handled entirely by backend)
  const chargeSavedCard = async () => {
    ...
    try {
      const response = await axios.post(
        ...
        {
          payment_method_id: selectedMethod.id,  // Send only the ID, not the authorization code
          amount,
          email,
          request_id: requestId,
          currency,
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="flex space-x-3">
        <button
          onClick={useNewCard ? initializePayment : chargeSavedCard}
          disabled={loading}
      ...
      <div className="mt-6 text-center">
        ...
        <p className="text-lg font-bold text-purple-600">Paystack</p>
      </div>
    </div>
  );
};

export default PaystackPaymentSecure;