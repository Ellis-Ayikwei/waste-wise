...
from .serializer import (
    ...
    PaystackPaymentCreateSerializer,
    PaystackPaymentUpdateSerializer,
    PaystackPaymentMethodSerializer,
    PaystackCustomerSerializer,
    ...
    def charge_authorization(self, request):
        ...
        try:
            ...
            if payment_method.user != user:
                logger.warning(
                    f"User {user.id} attempted to use payment method {payment_method_id} owned by user {payment_method.user.id}"
                )
                return Response(
                    {"success": False, "message": "Unauthorized payment method"},
                    status=status.HTTP_403_FORBIDDEN,
                )
            ...
            payment = Payment.objects.create(
                ...
                status=(
                    "success" if paystack_response["status"] == "success" else "failed"
                ),
                payment_type=data.get("payment_type", "full_payment"),
                ...
                paid_at=(
                    timezone.now() if paystack_response["status"] == "success" else None
                ),
                metadata=data.get("metadata", {}),
            )
            ...
            logger.info(
                f"Charged authorization for user {user.id}: {reference} - Status: {payment.status}"
            )
    ...
    def verify_payment(self, request):
        ...
        try:
            ...
            if payment.user != request.user:
                logger.warning(
                    f"User {request.user.id} attempted to verify payment {reference} owned by user {payment.user.id}"
                )
                return Response(
                    {"success": False, "message": "Unauthorized"},
                    status=status.HTTP_403_FORBIDDEN,
            ...
            if paystack_response["status"] == "success":
                ...
                payment.fees = (
                    Decimal(str(paystack_response.get("fees", 0))) / 100
                )  # Convert from kobo
                ...
                payment.gateway_response = paystack_response.get(
                    "gateway_response", "Payment verification failed"
                )
            ...
            logger.info(
                f"Payment verified for user {request.user.id}: {reference} - Status: {payment.status}"
            )
    ...
    def payment_methods(self, request):
        ...
        try:
            ...
            methods = PaymentMethod.objects.filter(
                user=user, is_active=True, reusable=True
            )
...
def paystack_webhook(request):
    ...
    try:
        ...
        webhook = PaymentWebhook.objects.create(
            event=event, reference=webhook_data.get("reference", ""), payload=data
        )
