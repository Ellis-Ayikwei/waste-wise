...
class ChatModelTests(TestCase):
    ...
    def test_create_conversation(self):
        ...
        conversation = Conversation.objects.create(
            title="Test Conversation", conversation_type="general"
        )
...
class ChatAPITests(APITestCase):
    ...
    def test_create_conversation(self):
        ...
        response = self.client.post("/wasgo/api/v1/chat/conversations/", data)
    ...
    def test_send_message(self):
        ...
        # Create conversation
        conversation = Conversation.objects.create(
            title="Message Test", conversation_type="general"
        )
        conversation.add_participant(self.user1)
        conversation.add_participant(self.user2)
        ...
        response = self.client.post("/wasgo/api/v1/chat/messages/", data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_list_conversations(self):
        ...
        response = self.client.get("/wasgo/api/v1/chat/conversations/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            len(response.data["results"]), 2
        )  # User1 is in 2 conversations

    def test_conversation_permissions(self):
        """Test that users can only see conversations they're part of"""
        conversation = Conversation.objects.create(
            title="Private Conv", conversation_type="general"
        )
        conversation.add_participant(self.user1)
        conversation.add_participant(self.user2)
        ...
        response = self.client.get(
            f"/wasgo/api/v1/chat/conversations/{conversation.id}/"
        )
    ...
    def test_mark_messages_read(self):
        ...
        conversation = Conversation.objects.create(
            title="Read Test", conversation_type="general"
        )
        conversation.add_participant(self.user1)
        conversation.add_participant(self.user2)
        ...
        msg1 = ChatMessage.objects.create(
            conversation=conversation, sender=self.user1, content="Message 1"
        )
        msg2 = ChatMessage.objects.create(
            conversation=conversation, sender=self.user1, content="Message 2"
        )
        ...
        response = self.client.post(
            "/wasgo/api/v1/chat/messages/mark_multiple_read/", data
        )
    ...
    def test_support_conversation(self):
        ...
        response = self.client.post(
            "/wasgo/api/v1/chat/conversations/create_support_conversation/", data
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
