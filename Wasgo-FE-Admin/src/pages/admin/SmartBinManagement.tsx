...
const SmartBinManagement: React.FC = () => {
    ...
    // Fetch data
    const { data: binsData, isLoading, mutate } = useSwr<SmartBin[]>('admin/smart-bins', fetcher);
    ...
    const mockBins: SmartBin[] = [
        ...
        {
            id: '3',
            name: 'Business District Bin #3',
            location: 'Business District, Financial Street',
            coordinates: { lat: 40.7069, lng: -74.0090 },
            fillLevel: 95,
            temperature: 21,
            batteryLevel: 65,
            signalStrength: 70,
            ...
            alerts: [
                {
                    ...
                    type: 'high_fill',
                    severity: 'critical',
                    message: 'Fill level above 90% - Immediate attention required',
            ...
            maintenanceHistory: [
                {
                    id: '2',
                    type: 'emergency',
                    description: 'Emergency maintenance due to sensor malfunction',
                    date: '2024-01-20',
    ...
    const bins = binsData || mockBins;
    const routes = routesData || mockRoutes;
    ...
    const stats = {
        ...
        onlineBins: bins.filter(bin => bin.status === 'online').length,
        offlineBins: bins.filter(bin => bin.status === 'offline').length,
        maintenanceBins: bins.filter(bin => bin.status === 'maintenance').length,
        criticalAlerts: bins.reduce((count, bin) => 
            count + bin.alerts.filter(alert => alert.severity === 'critical' && !alert.resolved).length, 0
        ),
        averageFillLevel: bins.reduce((sum, bin) => sum + bin.fillLevel, 0) / bins.length,
        averageBatteryLevel: bins.reduce((sum, bin) => sum + bin.batteryLevel, 0) / bins.length,
    };
